...
| **file**            | Optional global log file for all loggers. Relative path (`/logs/game.log`) → project folder when the game runs from the project root (the editor), save directory otherwise. The save directory has no nested folders (`sys.get_save_file`), so `/` is replaced with `_` (`logs_game.log`). Empty disables. Also via `log.set_file(path)`. | _(empty)_ |
